 ...
 * revision can move them - which also means this repo has no business stating
 * them. The chip's own SDK already does, per target, under
 * `components/soc/<IDF_TARGET>/include/soc/`; `bsp.c` includes those headers.
 * A copy here would be a second answer that can disagree with the first. */
