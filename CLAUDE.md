...
slot, and confirms or rolls back on the next boot. Product id **0xF001**; the
build entry is `workspace/0xF001/`.

Source is written against the house embedded-C standard at
`E:\Baotd\docs\embedded-spec`. A comment citing `R-XXX-nn` resolves to a rule
there — see `docs/memory-ai/rule/coding-standard-source.md`.
...
Never guess a build command — the repo root has no `CMakeLists.txt`. Use the
wrapper, which pins the workspace:

```bash
python docs/scripts/tool-esp.py build
python docs/scripts/tool-esp.py flash --port COM7
python docs/scripts/tool-esp.py format
```

Formatting is not a review topic: `clang-format` decides, and the pre-commit
hook in `docs/.githooks/` enforces it. Install it once with
